...
class ColorBalancer(object):
    def __init__(self, mainWindowName, mainWidth, mainHeight):
        self.windowName = "color balancer"
        self.frame = None
        ...
        self.delta = numpy.array([0] * 3)

        cv2.namedWindow(self.windowName)

        cv2.createTrackbar(self.trackBars['redBalance'], self.windowName, 255, 255, self.updateBalance)
        cv2.createTrackbar(self.trackBars['greenBalance'], self.windowName, 255, 255, self.updateBalance)
        cv2.createTrackbar(self.trackBars['blueBalance'], self.windowName, 255, 255, self.updateBalance)

        cv2.moveWindow(self.windowName, 0, int(mainHeight) + 220)
        cv2.resizeWindow(self.windowName, int(mainWidth), 200)

        cv2.setMouseCallback(mainWindowName, ColorBalancer.balanceToTrackbarColor, self)

    @staticmethod
    def balanceToTrackbarColor(event, x, y, flags, self):
        if event == cv2.EVENT_LBUTTONDOWN:#if flags & cv2.EVENT_FLAG_LBUTTON:
            red, green, blue = self.frame[y][x]

            actualR = cv2.getTrackbarPos(self.trackBars['redBalance'], self.windowName)
            actualG = cv2.getTrackbarPos(self.trackBars['greenBalance'], self.windowName)
            actualB = cv2.getTrackbarPos(self.trackBars['blueBalance'], self.windowName)

            self.delta = numpy.array([actualB - blue, actualG - green, actualR - red])


    def updateBalance(self, frame):
        self.frame = frame
        self.r = cv2.getTrackbarPos(self.trackBars['redBalance'], self.windowName)
        self.g = cv2.getTrackbarPos(self.trackBars['greenBalance'], self.windowName)
        self.b = cv2.getTrackbarPos(self.trackBars['blueBalance'], self.windowName)
...
def getSignificantColors(frame):
    color = ('r', 'g', 'b')
