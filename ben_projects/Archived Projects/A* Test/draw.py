...
class Drawer(object):
    ...
    def __init__(self, gridWidth, gridHeight, width, height, start, end):
        ...
        self.grid = numpy.zeros((self.gridHeight, self.gridWidth), dtype=numpy.int8)  # init empty grid
    ...
    def initGridDisplay(self, gridWidth, gridHeight):
        ...
        self.window = (100, 100)  # how much of the grid to show at a time
    ...
    def updateVisible(self):
        self.visibleGrid = self.grid[self.offset[1]:self.offset[1] + self.window[1],
                                     self.offset[0]:self.offset[0] + self.window[0]]  # crop grid
    
    def connectCells(self, cellX0, cellY0, cellX1, cellY1, color=(0, 0, 0), width=2):
    ...
    def enumerateGrid(self):
        if self.showFull == False:
            return numpy.ndenumerate(self.visibleGrid)
        else:
            return numpy.ndenumerate(self.grid)
        ...
        while self.done is False:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    self.brushMode = not self.grid[gridY][gridX]
                    ...
                    if event.key == pygame.K_SPACE:
                        if len(self.waypoints) == 0:
                            self.path, self.gridInfo, time = astar.search(self.grid, self.start, self.end)  # find a path between the start and end
                        else:
                            self.path, self.gridInfo, time = astar.searchWaypoints(self.grid, [self.start] + self.waypoints + [self.end])  # find a path between the start, waypoints, and the end
                        print time
                    ...
                    # Fill the entire grid
                        ...
                        self.grid = numpy.ones((self.gridHeight, self.gridWidth))
                        self.grid[self.start[1], self.start[0]] = 0
                        self.grid[self.end[1], self.end[0]] = 0
                        self.updateVisible()
                    ...
                    # reset grid to initial state
                        ...
                        self.grid = numpy.zeros((self.gridHeight, self.gridWidth))
                        self.path = []
                        self.waypoints = []
                        self.gridInfo = {}
                        self.updateVisible()
                    ...
                    # delete waypoint nearest to mouse
                        ...
                        for index, (wpY, wpX) in enumerate(self.waypoints):
                            distance = Drawer.pythagoreanDist(wpX, wpY, cellX, cellY)
                        ...
                        if numpy.all(self.grid == 0):
                            self.grid = mapmaker.read()
                            self.initGridDisplay(self.grid.shape[1], self.grid.shape[0])
                            self.updateVisible()
                        else:
                            mapmaker.write(self.grid)
                ...
                if self.mouseDown == True:
                    ...
                    for gridY in xrange(max(clickedY - radius + self.brushSize % 2, 0),
                                      min(clickedY + radius, self.gridHeight)):
                        for gridX in xrange(max(clickedX - radius + self.brushSize % 2, 0),
                                          min(clickedX + radius, self.gridWidth)):
                            self.grid[gridY][gridX] = self.brushMode
            ...
            pygame.draw.rect(self.screen, Drawer.BLACK,
                             [0, 0, self.cellSize * self.gridWidth, self.cellSize * self.gridHeight], 1)
            self.clock.tick(30)
