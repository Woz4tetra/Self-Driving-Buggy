...
def overlayCameras(frame1, frame2):
    height1, width1 = frame1.shape[0:2]
    height2, width2 = frame2.shape[0:2]
    rect2 = (0, 0, width2, height2)

    tracker = PlaneTracker()
    tracker.clear()
    tracker.add_target(frame2, rect2)

    tracked = tracker.track(frame1)
    for tr in tracked:
        cv2.polylines(frame1, [numpy.int32(tr.quad)], True, (255, 255, 255), 2)
        for (x, y) in numpy.int32(tr.p1):
            cv2.circle(frame1, (x, y), 4, (255, 255, 255), 2)
    if len(tracked) > 0:
        srcPoints = numpy.float32([
            [0, 0],
            [width2 - 1, 0],
            [width2 - 1, height2 - 1],
            [0, height2 - 1]
        ])
        M = cv2.getPerspectiveTransform(srcPoints, tracked[0].quad)
        overlay = cv2.warpPerspective(frame2, M, (width1, height1))
        thresh, mask = cv2.threshold(cv2.cvtColor(overlay, cv2.COLOR_BGR2GRAY), 1, 255, cv2.THRESH_BINARY)

        frame1 = cv2.subtract(frame1, cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR))
        frame1 = cv2.add(frame1, overlay)
    return frame1
    # else:
    #     return frame2


def balanceFrame(frame):
    # min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY))
    # print(frame[max_loc[1], max_loc[0]], frame[min_loc[1], min_loc[0]])
    #
    # cv2.circle(frame, max_loc, 5, (255, 100, 100), 5)
    #
    # height, width = frame.shape[0:2]
    # frame = frame[height / 2:, :]
    # # satLevel = 50
    # # q = [satLevel / 2, 100 - satLevel / 2]
    # colors = ['r', 'g', 'b']
    # for color in xrange(3):
    #     # tiles = numpy.percentile(frame[:, :, color], q)
    #     # print tiles, q
    #     hist = cv2.calcHist(frame, [0], None, [256], [0, 256])
    #     minimum = numpy.min(frame[:, :, color])
    #     maximum = numpy.max(frame[:, :, color])
    #     frame[:, :, color] = numpy.int8((numpy.float32(frame[:, :, color]) - minimum) * 255 / (maximum - minimum))
    #     hist2 = cv2.calcHist(frame, [0], None, [256], [0, 256])
    #
    #     plot.subplot(221), plot.plot(hist, color=colors[color])
    #     plot.subplot(222), plot.plot(hist2, color=colors[color])
    #     plot.xlim([0, 256])
    #
    # plot.show()
...
def filterOutNonAsphalt(road):
    ...
    gray = contrast(gray, 2)

    gray = cv2.GaussianBlur(gray, (3, 3), 0)
    # gray = cv2.medianBlur(gray, 3)
    # gray = cv2.Sobel(gray, cv2.CV_8UC1, 0, 1, ksize=5)
    threshVal, gray = cv2.threshold(gray, 210, 255, cv2.THRESH_BINARY)
...
def findStreetCenterAndEdges(contours, (height, width)):
...
def perspectiveWarpRoad(road, points, (height, width)):
    ...
    return cv2.warpPerspective(road, M, (width, height), road)


#       - - - - - - - - - - - - - - - - - - - - - - - - -
#                       filterOutNonAsphalt
#       - - - - - - - - - - - - - - - - - - - - - - - - -
def reduceColors(frame, div=16):
    frame = frame / div * div + div / 2
    return frame


def generateColors(numberOfColors, max):
    num = math.ceil(numberOfColors ** (1.0 / 3))
    return numpy.int32(list(numpy.ndindex(num, num, num))) * (max / (num - 1))


def clipImage(image, lower, upper):
    image = numpy.clip(image, lower, upper)
    image -= lower
    image *= 255.0 / (upper - lower)
