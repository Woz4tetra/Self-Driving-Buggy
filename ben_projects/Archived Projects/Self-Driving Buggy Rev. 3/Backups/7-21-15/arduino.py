...
class Device(object):
    ...
    def __init__(self, serialFlags, pin=None, pinRange=None):
        '''
        This class and all subclasses act as wrappers for serial. Here's a
         ...
         various parts of the arduino code.
            ...
            Example 1: Single command to return data
        ...
        If a subclass of Device needs multiple commands, you can provide a
         dictionary to the serialFlags parameter. The best example is the
         DCMotor class. This appears in its constructor:
             super(DCMotor, self).__init__({
                True: 'c',  # forward
                False: 'd',  # backward
                "stop": 'e'
             }, pin=pin, pinRange=(1, 4))
         So if you want to send the forward flag, call self.writeFlag(True).


            Example 3: Series of commands
        ...
        Some subclasses Device require data (like pin numbers and other data)
         ...
         too. This is arduino sketch for the forward motor command:
        ...
        '''
        if hasattr(serialFlags, '__iter__'):
            Device._checkFlags(serialFlags.values())
            self.serialFlags = serialFlags
        else:
            Device._checkFlags([serialFlags])
            self.serialFlags = {}

            self.serialFlags[None] = serialFlags
    ...
    def writeFlag(self, flagName=None):
        '''
        ...
        :return: None
        '''
        arduino.writeSerial(self.serialFlags[flagName])
...
class DCMotor(Device):
    def __init__(self, pin):
        ...
        for its constructor for more details. Uses the 'c', 'd', and 'e' flags.
    ...
    def speed(self, value):
        '''
        Speed is an internal variable that should only be a range from
        0...255

        :param value: the new speed of the motor
        :return: None
        '''
        if value > 255:
            self._speed = 255
    ...
    def reverse(self):
        '''
        Reverses the self.forward variable which acts as the key for
        self.serialFlags.
    ...
    def changeSpeed(self, deltaSpeed):
        '''
        Change self.speed by deltaSpeed
    ...
    def _drive(self):
        '''
        An internal method. Sends the forward or backward flag according to
        self.forward, then the pin, and finally the speed to serial in that
        order.
    ...
    def driveForward(self, speed):
        '''
        Use this method to control the forward direction of the motor (as
        opposed to _drive())

        :param speed: a number from 0...255
        :return: None
        '''
        self.speed = speed
        self.forward = True
    ...
    def driveBackward(self, speed):
        '''
        Use this method to control the backward direction of the motor (as
        opposed to _drive())
    ...
    def stop(self):
        '''
        Sends the stop flag to the motor
...
class Button(Device):
    def __init__(self, pin):
        ...
        self.previous = False
        self.state = False
        super(Button, self).__init__('f', pin=pin, pinRange=(0, 13))
...
class Sonar(Device):
    def __init__(self, pin):
        ...
        '''
        super(Sonar, self).__init__('g', pin, (0, 13))
