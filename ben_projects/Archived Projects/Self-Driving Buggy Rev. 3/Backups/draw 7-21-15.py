...
class Drawer(object):
    ...
    def __init__(self, gridWidth, gridHeight, width, height, start, end, grid=None):
        self.width = width
        self.height = height
        self.margin = 1
        ...
        if grid is None:
            self.grid = numpy.zeros((self.gridHeight, self.gridWidth, 2))  # init empty grid
        else:
            self.grid = grid
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    if len(self.waypoints) == 0:
                        self.path, self.gridInfo, plotTime = astar.search(self.grid, self.start, self.end)  # find a path between the start and end
                    else:
                        self.path, self.gridInfo, plotTime = astar.searchWaypoints(self.grid, [self.start] + self.waypoints + [self.end])  # find a path between the start, waypoints, and the end
                ...
                # reset grid to initial state
                    ...
                    self.grid = numpy.zeros((self.gridHeight, self.gridWidth, 2))
                    self.path = []
                    self.waypoints = []
                    self.gridInfo = {}
                    self.updateVisible()
