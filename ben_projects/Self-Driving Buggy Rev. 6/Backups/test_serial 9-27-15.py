...
if __name__ == '__main__':
    ...
    try:
        ...
        if board_type == 'pyboard':
            ...
            counter = 0
            while counter < 100:
                # in_packet_raw = communicator.get()
                in_packet_raw = ''
                char = ''
                print(counter)
                while char != '\n':
                    char = communicator.serialRef.read()
                    in_packet_raw += char
                print(in_packet_raw)
                in_packet = parser.parse(in_packet_raw)
                if in_packet != None:
                    node, command_id, payload = in_packet
                    if parser.verify(packet, in_packet):
                        counter += 1
                        print "in_packet_raw:", in_packet_raw

            # communicator.dump()

            # ---- Request data from fake sensor 16 bit ----
            packet = communicator.makePacket(PACKET_TYPES['request data'],
                                             ARDUINO_COMMAND_IDS[
                                                 'fake sensor 16 bit'])
            print "packet:", repr(packet)
            communicator.put(packet)
            for counter in xrange(14):
                in_packet_raw = communicator.get()
                in_packet = parser.parse(in_packet_raw)
                if in_packet != None:
                    node, command_id, payload = in_packet
                    ...
                    assert node == NODE_BOARD
                    assert command_id == ARDUINO_COMMAND_IDS[
                        'fake sensor 16 bit']
