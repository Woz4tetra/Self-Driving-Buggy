...
def filterOutNonAsphalt(road):
    flattenedRoad = numpy.float32(road.reshape(-1, 3))
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 5, 1.0)
    K = 3
    ret, label, center = cv2.kmeans(flattenedRoad, K, criteria, 5, cv2.KMEANS_RANDOM_CENTERS)

    center = numpy.uint8(center)
    res = center[label.flatten()]
    res2 = res.reshape((road.shape))

    cv2.imshow("road", res2)
